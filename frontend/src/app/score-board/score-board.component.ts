...
interface AdaptiveGuidanceWebsocket {
  challengeKey: string
  message: string
  level: number
}
...
export class ScoreBoardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit (): void {
    ...
    this.io.socket().emit('user connected', localStorage.getItem('token'))
    this.io.socket().on('adaptive guidance', this.onAdaptiveGuidanceWebsocket.bind(this))
  }
  ...
  ngOnDestroy (): void {
    ...
    this.io.socket().off('adaptive guidance', this.onAdaptiveGuidanceWebsocket.bind(this))
  ...
  onAdaptiveGuidanceWebsocket (data?: AdaptiveGuidanceWebsocket) {
    if (!data?.challengeKey || !data?.message) {
      return
    }

    this.allChallenges = this.allChallenges.map((challenge) => {
      if (challenge.key === data.challengeKey) {
        return {
          ...challenge,
          adaptiveGuidance: {
