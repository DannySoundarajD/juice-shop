...
describe('CodingChallengeFixItComponent', () => {
  ...
  it('should check fix and set wrong verdict', () => {
    codeFixesService.check.and.returnValue(of({ verdict: false, explanation: 'Wrong!', adaptiveHint: 'Use ownership checks.' }))
    component.randomFixes = [{ fix: 'fix1', index: 0 }]
    component.selectedFix = 0
    component.checkFix()
    expect(component.result).toBe(2)
    expect(component.explanation).toBeNull()
    expect(component.adaptiveHint).toBe('Use ownership checks.')
  })
  ...
  describe('template rendering', () => {
    ...
    it('should render adaptive guidance card when present', () => {
      component.result = 2
      component.explanation = null
      component.adaptiveHint = 'Use ownership checks.'
      fixture.changeDetectorRef.detectChanges()
      const card = fixture.nativeElement.querySelector('mat-card')
      expect(card).not.toBeNull()
      expect(card.textContent).toContain('Adaptive guidance:')
      expect(card.textContent).toContain('Use ownership checks.')
    })
  })
