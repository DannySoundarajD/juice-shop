...
export class CodingChallengeFixItComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public adaptiveHint: string = null
  ...
  checkFix (): void {
    this.codeFixesService.check(this.challengeKey(), this.randomFixes[this.selectedFix].index).subscribe((verdict) => {
      ...
      this.adaptiveHint = verdict.adaptiveHint ?? this.getAdaptiveHintFallback(verdict.verdict)
      this.explanation = this.adaptiveHint ? null : verdict.explanation
    })
  }

  shuffle (): void {
    if (this.challengeKey() === 'forgedReviewChallenge') {
      this.randomFixes = this.fixes().map((fix, index) => ({ fix, index }))
      return
    }
  ...
  private getAdaptiveHintFallback (verdict: boolean): string | null {
    if (verdict || this.challengeKey() !== 'forgedReviewChallenge') {
      return null
    }

    const originalFixIndex = this.randomFixes[this.selectedFix]?.index
    if (originalFixIndex === 0) {
