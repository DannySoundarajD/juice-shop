...
describe('CodingChallengeFindItComponent', () => {
  ...
  it('should check lines and set hint on wrong verdict', () => {
    vulnLinesService.check.and.returnValue(of({ verdict: false, hint: 'Try again', adaptiveHint: 'Look at the query.' }))
    component.checkLines()
    expect(component.result).toBe(2)
    expect(component.hint).toBe('Try again')
    expect(component.adaptiveHint).toBe('Look at the query.')
  })
  ...
  describe('hint card rendering', () => {
    ...
    it('should render hint card when hint is set', () => {
      vulnLinesService.check.and.returnValue(of({ verdict: false, hint: 'Look closer', adaptiveHint: 'Look at the query.' }))
      component.checkLines()
      fixture.detectChanges()
      const cards = fixture.nativeElement.querySelectorAll('mat-card')
      expect(cards.length).toBe(2)
      expect(cards[0].textContent).toContain('Look closer')
      expect(cards[1].textContent).toContain('Adaptive guidance:')
      expect(cards[1].textContent).toContain('Look at the query.')
    })
  })
