...
export class CodingChallengeFindItComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  checkLines (): void {
    this.vulnLinesService.check(this.challengeKey(), this.selectedLines).subscribe((verdict: result) => {
      ...
      const adaptiveHint = verdict.adaptiveHint ?? this.getAdaptiveHintFallback(verdict.verdict)
      this.hint = adaptiveHint ? null : verdict.hint
  ...
  private getAdaptiveHintFallback (verdict: boolean): string | null {
    if (verdict || this.challengeKey() !== 'forgedReviewChallenge' || this.selectedLines.length === 0) {
      return null
    }

    if (this.selectedLines.length === 1 && this.selectedLines[0] === 3) {
    ...
    if (this.selectedLines.length === 1 && this.selectedLines[0] === 4) {
